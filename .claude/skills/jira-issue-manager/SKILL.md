...
4. All commands run from the skill directory: `.claude/skills/jira-issue-manager/`
...
`<PROJECT_ROOT>/.claude/skills/jira-issue-manager/scripts/_adhoc.py`
...
**Step 2.** Run it:
```bash
uv run --native-tls --env-file ../../../.env scripts/_adhoc.py
```

**Step 3.** Delete the file after execution:
```bash
rm <PROJECT_ROOT>/.claude/skills/jira-issue-manager/scripts/_adhoc.py
```
...
Run from the skill directory (`.claude/skills/jira-issue-manager/`):

```bash
uv run --native-tls --env-file ../../../.env scripts/<script_name>.py [args]
```
