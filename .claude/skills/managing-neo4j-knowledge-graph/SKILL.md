...
```bash
uv run --native-tls --env-file .claude/skills/managing-neo4j-knowledge-graph/scripts/.env .claude/skills/managing-neo4j-knowledge-graph/scripts/<script_name>.py
```
...
- **`.claude/skills/managing-neo4j-knowledge-graph/scripts/health_check.py`** — Verify connectivity to the Neo4j instance. Run this to confirm the database is reachable and credentials are valid.
