...
Each sub-skill invocation MUST be a **fresh agent instance** (use the Agent tool). Do NOT reuse conversation context between sub-task steps. This ensures:
...
**Sub-task**: Use `/jira-issue-manager` to search for all issues in the sprint.
...
**Sub-task**: Use `/jira-issue-manager` to search for completed issues.
...
**Sub-task**: Use `/jira-issue-manager` to search for incomplete issues.
